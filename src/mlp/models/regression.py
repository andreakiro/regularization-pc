...
class PCSimpleRegressor(nn.Module):
    ...
    def __init__(self, dropout: float = 0.0, input_dim: int = 1) -> None:
        ...
        self.f = torch.relu
        self.f_prime = lambda x: torch.stack(
            [torch.relu(torch.sign(torch.diag(x[i, :, 0]))) for i in range(x.shape[0])])
    ...
    def backward_x(self):
        ...
        """
        self.grad_x = Parallel(n_jobs=len(
            self.pc_layers[:-1]))(delayed(self.grad_xi)(i) for i in range(len(self.pc_layers[:-1])))
    ...
    def backward_w(self):
        ...
        """
        self.grad_w = Parallel(n_jobs=len(self.linear_layers))(
            delayed(self.grad_wi)(i) for i in range(len(self.pc_layers)))
    ...
    def step_x(self, η):
        ...
        """
        new_x = Parallel(n_jobs=len(self.pc_layers[:-1]))(
            delayed(self.step_xi)(i, η) for i in range(len(self.pc_layers[:-1])))
    ...
    def step_w(self, η):
        ...
        """
        new_w = Parallel(n_jobs=len(self.linear_layers))(
            delayed(self.step_wi)(i, η) for i in range(len(self.linear_layers)))
    ...
    def grad_xi(self, i):
        ...
        """
        with torch.no_grad():
            e_i, e_ii = self.pc_layers[i].ε.detach(
            )[:, :, None], self.pc_layers[i+1].ε.detach()[:, :, None]
            w_ii = self.linear_layers[i+1].weight.detach()[None,
                                                           :, :].expand(e_i.shape[0], -1, -1)
            x_i = self.pc_layers[i].x.detach()[:, :, None]
            grad = e_i - \
                (torch.transpose(self.f_prime(w_ii @ x_i) @ w_ii, 1, 2) @ e_ii)
            return grad
    ...
    def grad_wi(self, i):
        ...
        with torch.no_grad():
            ...
            w_i = self.linear_layers[i].weight.detach(
            )[None, :, :].expand(e_i.shape[0], -1, -1)
            x_i = self.pc_layers[i-1].x.detach()[:, :,
                                                 None] if i > 0 else self.input.detach()[:, :, None]
